...
flowchart LR
  ...
  S2 --> S3[3. 制御構文]
  S2 --> S4[4. 配列とリスト]
  S3 --> S31[3.1. Null許容型と例外処理]
  S31 --> S32[3.2. システム例外とビジネス例外]
  S4 --> S7[7. LINQ]
  S2 --> S5[5. オブジェクト指向]
  S5 --> S51[5.1. メソッドとスコープ]
  S5 --> S52[5.2. インターフェース]
  S2 --> S6[6. 標準入出力]
  S6 --> S61[6.1. ストリーム]
...
| **3.1** | [Null許容型と例外処理](./docs/03_try_catch.md) | Null許容型と例外処理を扱う | ✅ |
| **3.2** | [システム例外・ビジネス例外](./docs/03_exception.md) | システム例外とビジネス例外を区別する | ✅ |
| **4** | [配列とリスト](./docs/04_collections.md) | データの集合を扱う (List<T>) | ✅ |
| **5** | [オブジェクト指向](./docs/05_oop.md) | クラス・継承・カプセル化 | ✅ |
| **5.1** | [メソッドとスコープ](./docs/05_method.md) | メソッドとスコープを理解する | ✅ |
| **5.2** | [インターフェース](./docs/05_interface.md) | クラスへのインターフェスの実装 | ✅ |
...
| **6.3** | [JSON](./docs/06_json.md) | JSONシリアライズ・デシリアライズ | ✅ |
